...
В данной лабораторной работе были изучены основные свойства и возможности LinearLayout и ConstraintLayout. Для этого создавались макеты по вариантам.  Были рассмотрены разные варианты решения задач - с использованием элементов space, guiedline и chain. На практике было проверено, что Constraintlayout позволяет легко создавать довольно сложные макеты благодаря гибкому размещению объектов. Таким образом, ConstraintLayout применяется чаще, так как использует плоскую иерархию представлений (создание сложной композиции с помощью LinearLayout ухудшает производительность приложения, так как используются вложенные структуры). 
